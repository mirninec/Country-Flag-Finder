
/**
 * @brief Загружает данные о флагах из локального файла JSON.
 * @return {Promise<Object>} Возвращает объект с данными о флагах.
 */
async function loadFlagsData() {
    const response = await fetch(chrome.runtime.getURL('convertedFlags.json'));
    return response.json();
}

const flags = loadFlagsData();  ///< Промис, который будет разрешен с данными флагов.



/**
 * @brief Инициализация расширения.
 * 
 * Получает список всех открытых вкладок, собирает информацию о сервере (страна, флаг), 
 * устанавливает иконки и названия для каждой вкладки, а также добавляет слушатели на обновление и активацию вкладок.
 */
async function initExt() {

    await chrome.storage.local.clear()

    /**
     * @brief Получение всех открытых вкладок браузера.
     * @return {Promise<Array>} Возвращает массив объектов вкладок.
     */
    const tabs = await new Promise((resolve) => {
        chrome.tabs.query({}, (result) => resolve(result));
    });

    // Обрабатываем каждую вкладку для сбора информации о сервере и флаге.
    tabs.map(async tab => {
        const result = await getFlagImage(new URL(tab.url).hostname);
        await updateTabInfo(tab, result);
    });
}

// Добавляем слушателя на обновление вкладок.
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    const idString = tabId.toString();

    chrome.storage.local.get([idString], async function (result) {
        if (changeInfo.status === 'complete' && tab.active) {
            const hostname = new URL(tab.url).hostname;
            const flagData = await getFlagImage(hostname);
            await updateTabInfo(tab, flagData);
        }
    });
});

// Добавляем слушателя на изменение активной вкладки.
/**
 * @brief Обрабатывает изменение активной вкладки и выводит информацию о ней в консоль.
 * @param {Object} activeInfo Информация об активной вкладке (ID и индекс).
 */
chrome.tabs.onActivated.addListener(async (activeInfo) => {
    const id = activeInfo.tabId.toString();

    chrome.storage.local.get([id], function (result) {
        if (result) {
            console.log(`Вкладка ${id} есть в chrome.storage.local`, result);
        } else {
            console.log(`Вкладки ${id} ещё нет в chrome.storage.local`);
        }
    });
});

/**
 * @brief Получает изображение флага и информацию о стране по хостнейму.
 *        Также возвращает IP-адреса сервера.
 * @param {string} hostname Хостнейм сайта.
 * @return {Promise<Object>} Возвращает объект с изображением флага, названием страны и IP-адресами.
 */
async function getFlagImage(hostname) {
    // Для браузерных страниц возвращаем специальные данные.
    if (hostname === "extensions") {
        return {
            countryName: "браузер Chrome",
            flagImg: brauserChromeImg,
            ips: ["127.0.0.1"]
        };
    }

    // Другие случаи (например, пустой hostname или newtab).
    if (hostname === "" || hostname === "newtab" || hostname === "history") {
        return {
            countryName: "локальная страница",
            flagImg: osWindowsImg,
            ips: ["127.0.0.1"]
        };
    }

    try {
        // Запрашиваем IP-адрес и информацию о стране для данного хостнейма.
        const ipResponse = await fetch(`https://linux-academy.ru/what-is-country/${hostname}`);
        const ipData = await ipResponse.json();

        // Возвращаем данные о стране и IP-адресах.
        if (ipData) {
            return ipData;
        }
    } catch (error) {
        console.log('Ошибка при получении страны или IP:', error);
        return {
            countryName: 'Error',
            flagImg: osWindowsImg,
            ips: ["127.0.0.1"] // Возвращаем localhost в случае ошибки.
        };
    }
}

chrome.tabs.onRemoved.addListener(function (tabId) {
    const id = tabId.toString();

    chrome.storage.local.remove([id], function () {
        console.log(`Вкладка ${id} была удалена из chrome.storage.local`);
    });
});

/**
 * @brief Получает IP всех открытых вкладок, проверяет наличие данных в chrome.storage.local
 *        с ключами, равными этим IP, и удаляет данные для IP, которые не соответствуют
 *        ни одной из открытых вкладок.
 */
async function syncStorageWithTabs() {
    // Шаг 1: Получаем все открытые вкладки.
    const tabs = await new Promise((resolve) => {
        chrome.tabs.query({}, (result) => resolve(result));
    });

    // Шаг 2: Получаем IP-адреса для всех вкладок.
    let tabIPs = [];

    for (let tab of tabs) {
        try {
            const url = new URL(tab.url); // Извлекаем хостнейм из URL вкладки.
            const result = await getFlagImage(url.hostname); // Получаем данные, включая IP-адрес.
            if (result && result.ips) {
                // Добавляем все IP-адреса, связанные с этой вкладкой.
                tabIPs = tabIPs.concat(result.ips);
            }
        } catch (error) {
            console.error(`Ошибка при получении IP для вкладки ${tab.id}:`, error);
        }
    }

    // Удаляем дубликаты IP-адресов, если такие есть.
    tabIPs = [...new Set(tabIPs)];

    // Шаг 3: Получаем все данные из chrome.storage.local.
    chrome.storage.local.get(null, function (items) {
        const storedKeys = Object.keys(items); // Все ключи, сохранённые в хранилище.

        // Шаг 4: Находим ключи, которые не соответствуют IP-адресам открытых вкладок.
        const keysToDelete = storedKeys.filter(key => !tabIPs.includes(key));

        // Шаг 5: Удаляем данные из хранилища для ключей, которые не соответствуют активным IP.
        if (keysToDelete.length > 0) {
            chrome.storage.local.remove(keysToDelete, function () {
                console.log(`Удалены данные для следующих ключей (IP):`, keysToDelete);
            });
        } else {
            console.log('Все данные соответствуют активным IP, удаление не требуется.');
        }
    });
}

/**
 * @brief Обновляет информацию о вкладке, устанавливает флаг и название страны в качестве иконки и заголовка вкладки,
 *        а также сохраняет данные о вкладке в локальное хранилище Chrome.
 * 
 * @param {Object} tab Объект вкладки, содержащий информацию о текущей вкладке браузера.
 * @param {Object} result Объект, возвращаемый функцией getFlagImage, содержащий флаг, имя страны и IP-адреса.
 * 
 * Функция выполняет следующие действия:
 * - Устанавливает иконку флага для вкладки.
 * - Устанавливает заголовок вкладки с указанием страны сервера.
 * - Извлекает дополнительную информацию о флаге, если она доступна.
 * - Сохраняет все данные о вкладке (флаг, страна, IP, иконка) в локальное хранилище Chrome для дальнейшего использования.
 */
async function updateTabInfo(tab, result) {

    console.log(`функция %cupdateTabInfo\x1b[0m  вызвана для вкладски с id %c${tab.id}`, baseStyles, greenStyles)

    // Устанавливаем иконку флага для вкладки, используя путь к изображению в формате base64.
    chrome.action.setIcon({ path: "data:image/png;base64," + result.flagImg, tabId: tab.id });

    // Устанавливаем заголовок вкладки с указанием страны, в которой находится сервер сайта.
    chrome.action.setTitle({ title: `Сервер сайта находится в \n${result.countryName}`, tabId: tab.id });

    // Извлекаем последние два символа из названия страны для использования в качестве сокращённого кода страны.
    const shortCountryName = result.countryName.slice(-3, -1);

    // Ожидаем разрешения промиса, который загружает данные о флагах из JSON.
    const dataFlags = await flags;

    let bigFlagImg;

    // Проверяем, существует ли флаг в большом формате для сокращённого кода страны.
    // Если да, то сохраняем путь к изображению большого флага.
    if (dataFlags[shortCountryName]) {
        bigFlagImg = dataFlags[shortCountryName].bigFlagImg;
        // console.log("dataFlags[shortCountryName] ", dataFlags[shortCountryName])
    }

    // Преобразуем ID вкладки в строку для использования в качестве ключа при сохранении в хранилище.
    const idString = tab.id.toString();

    // Формируем объект с данными о вкладке, включая:
    // - хостнейм сайта
    // - изображение флага
    // - большое изображение флага (если доступно)
    // - IP-адреса сервера
    // - название страны
    const data = {
        hostname: new URL(tab.url).hostname,
        flagImg: result.flagImg,
        bigFlagImg,  // может быть неопределенным, если большого флага нет
        ips: result.ips,
        countryName: result.countryName
    };

    // Создаем объект для сохранения данных в формате, где ключом будет строковый идентификатор вкладки.
    const dataToStore = {
        [idString]: data
    };

    chrome.storage.local.set(dataToStore, () => {
        // console.log("Сохранено: ", dataToStore);
    });

    // проверяем существует ли запись с указанным ip в chrome.storage.local, если нет, то сохраняем вкрадку
    chrome.storage.local.get(idString, (result) => {
        if (!result) {
            chrome.storage.local.set(dataToStore, () => {
                console.log("Сохранено: ", dataToStore);
            });
        }
    })
}


// Инициализируем расширение при запуске.
initExt();



const brauserChromeImg = ""

const osWindowsImg =
    ''

const compBase64String = ''

let baseStyles = [
    "color: #fff",
    "background-color: #b1acac",
    "padding: 2px 4px",
    "border-radius: 2px"
].join(";");

let greenStyles = [
    "font-size: 14px",
    "color: #fff",
    "background-color: #92b38f",
    "padding: 2px 4px",
    "border-radius: 2px"
].join(";");